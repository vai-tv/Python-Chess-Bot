...
class Computer:
    ...
    def old_evaluate(self, board: chess.Board) -> float:
        """
        ...
        def evaluate_player(color: chess.Color) -> float:
            ...
            def forcing_moves_bonus() -> float:
                ...
                for move in board.legal_moves:
                    ...
                    if board.is_capture(move):
                        bonus += 1.5
                    # Reward moves that attack pinned pieces or pieces with limited defenders
                    to_square = move.to_square
                    piece = board.piece_at(to_square)
                    if piece is not None and piece.color != color:
                        defenders = board.attackers(piece.color, to_square)
                        total_defender_material = sum(self.MATERIAL[board.piece_at(square).piece_type] for square in defenders) # type: ignore

                        bonus += self.MATERIAL[piece.piece_type] * (10 / (total_defender_material + 1))
                        # Reward attacking pinned pieces
                        if board.is_pinned(not color, to_square):
                            bonus += self.MATERIAL[piece.piece_type] ** 1.25
                    board.pop()
                return bonus

            def defensive_moves_bonus() -> float:
                bonus = 0.0
                for square, piece in piece_map.items():
                    if piece.color == color:
                        ...
                        if attackers and defenders:
                            # Reward defending pieces under attack
                            bonus += self.MATERIAL[piece.piece_type] * 1.5
                return bonus

            def piece_placement_bonus() -> float:
                bonus = 0.0
                for square, piece in piece_map.items():
                    if piece.color == color:
                        rank = chess.square_rank(square)
                        file = chess.square_file(square)
                        # Reward central control for knights and bishops
                        if piece.piece_type in [chess.KNIGHT, chess.BISHOP]:
                            if 2 <= rank <= 5 and 2 <= file <= 5:
                                bonus += 1.5
                        # Reward rooks on open or semi-open files
                        if piece.piece_type == chess.ROOK:
                            file_pawns = [sq for sq in PAWNS[color] if chess.square_file(sq) == file]
                            enemy_file_pawns = [sq for sq in PAWNS[not color] if chess.square_file(sq) == file]
                            if len(file_pawns) == 0:
                                bonus += 2.0  # Open file
                            elif len(enemy_file_pawns) == 0:
                                bonus += 1.0  # Semi-open file
                return bonus
                ...
                for square in pawns:
                    ...
                    for adj_file in [file - 1, file + 1]:
                        ...
                        if connected:
                            break
                    if connected:
                        pawn_score += 2.0  # Increased reward for connected pawns
                    ...
                    for ep_file, ep_rank in zip(enemy_pawn_files, enemy_pawn_ranks):
                        if abs(ep_file - file) <= 1:
                            if (color == chess.WHITE and ep_rank > rank) or (color == chess.BLACK and ep_rank < rank):
                                ...
                                break
                    if is_passed:
                        pawn_score += 3.0  # Increased reward for passed pawns
                        if connected: # Reward connected passed pawns
                            pawn_score += 2.0  # Increased reward for connected passed pawns
                    ...
                    # Pawns are better when closer to the centre
                    pawn_score -= 0.5 * abs(file - 3.5)
            ...
            def attack_quality() -> float:
                ...
                for move in list(board.legal_moves):
                    ...
                    board.push(move)
                    if board.is_check():
                        aggression_score += 1.0  # Reduced reward for checks
                    # Reward captures
                    if board.is_capture(move):
                        aggression_score += 1.0  # Reduced reward for captures
                    ...
                    if enemy_king_square is not None:
                        ...
                        if enemy_king_square in attacked_squares:
                            aggression_score += 1.5  # Reduced reward for attacking king vicinity
                    board.pop()
                    ...
                    if piece.color != color:
                        ...
                        if attackers:
                            if len(defenders) == 0:
                                ...
                                aggression_score += self.MATERIAL[piece.piece_type] * len(attackers) * 3.0  # Reduced reward
                            else:
                                ...
                                if self.MATERIAL[piece.piece_type] < defender_value:
                                    aggression_score += self.MATERIAL[piece.piece_type] * len(attackers) * 1.5  # Reduced reward
                                else:
                                    # Penalize attacking well-defended pieces
                                    aggression_score -= self.MATERIAL[piece.piece_type] * len(attackers) * 2.0  # Increased penalty
                ...
                for move in board.legal_moves:
                    ...
                    if board.is_checkmate():
                        board.pop()
                        aggression_score += 8.0  # Reduced reward for checkmate threats
                        break
                    board.pop()
                ...
                for move in board.legal_moves:
                    ...
                    if victim and attacker:
                        ...
                        if self.MATERIAL[victim.piece_type] < self.MATERIAL[attacker.piece_type]:
                            aggression_score += 1.0  # Reduced reward
                        # Encourage trading down when ahead in material
                        if material_diff > 1.5:
                            aggression_score += 2.0  # Reduced encouragement
                        elif material_diff < 0.75:
                            aggression_score -= 10.0  # Increased penalty
            ...
            score += cse(attack_quality(), 1.2) * aggression[color] * 15
            score += cse(forcing_moves_bonus(), 1.2) * aggression[color]
            score += defensive_moves_bonus() * 10 * aggression[not color]
            score += piece_placement_bonus() * 10
            ...
            if isinstance(score, complex):
                ...
                print("+MS", cse(material_score(), 3) * 75)
                ...
                print("+PAWN", cse(pawn_structure(), 1.5) * 2.5 * (2 if stage == 'late' else 1.5))
                print("+ATT", cse(attack_quality(), 1.2) * aggression[color] * 5)
                print("+FORC", forcing_moves_bonus() * aggression[color])
                print("+DEF", defensive_moves_bonus() * 10 * aggression[color])
                print("+PLAC", piece_placement_bonus() * 10 * aggression[color])
    ...
    def evaluate(self, board: chess.Board) -> float:
        ...
        # Try to get score from DB
        score = self.evaluate_from_db(board)
        ...
        def evaluate_player(color: chess.Color) -> float:
            ...
            def material_score() -> float:
                # Base material score
                return material[color] * 100

            def mobility_score() -> float:
                # Reward mobility: number of squares attacked by pieces
                mobility = 0
                for square, piece in piece_map.items():
                    if piece.color == color:
                        attacks = board.attacks(square)
                        mobility += len(attacks) * (self.MATERIAL.get(piece.piece_type, 1) ** 0.5)
                return mobility * 0.1

            def king_safety() -> float:
                ...
                # Get squares around king (one square radius)
                king_file = chess.square_file(king_square)
            ...
            def pawn_structure() -> float:
                ...
                pawns = [sq for sq, p in piece_map.items() if p.color == color and p.piece_type == chess.PAWN]
                score = 0
                files = [chess.square_file(sq) for sq in pawns]
                for file in set(files):
                    count = files.count(file)
                    if count > 1:
                        score -= 10 * (count - 1)  # doubled pawns penalty
                for sq in pawns:
                    file = chess.square_file(sq)
                    # isolated pawn penalty
                    if (file == 0 or file - 1 not in files) and (file == 7 or file + 1 not in files):
                        score -= 15
                # passed pawns bonus
                enemy_pawns = [sq for sq, p in piece_map.items() if p.color != color and p.piece_type == chess.PAWN]
                enemy_files = [chess.square_file(sq) for sq in enemy_pawns]
                enemy_ranks = [chess.square_rank(sq) for sq in enemy_pawns]
                for sq in pawns:
                    file = chess.square_file(sq)
                    rank = chess.square_rank(sq)
                    passed = True
                    for ef, er in zip(enemy_files, enemy_ranks):
                        if abs(ef - file) <= 1:
            ...
            def piece_activity() -> float:
                # Reward pieces controlling center squares
                center_squares = [chess.D4, chess.D5, chess.E4, chess.E5]
                score = 0
                for sq, piece in piece_map.items():
                    if piece.color == color and sq in center_squares:
                        score += self.MATERIAL.get(piece.piece_type, 1) * 2
                return score

            def threats() -> float:
                # Reward attacking opponent pieces and checks
                score = 0
                enemy_color = not color
                for move in board.legal_moves:
                    target_piece = board.piece_at(move.to_square)
                    if target_piece is not None and target_piece.color == enemy_color:
                        score += self.MATERIAL.get(target_piece.piece_type, 1) * 1.5
                    board.push(move)
                    if board.is_check():
                        score += 10
                    board.pop()
            ...
            # Combine sub-scores with exponents and multipliers
            score = 0
            score += material_score()
            score += cse(mobility_score(), 1.5)
            score += cse(king_safety(), 1.2)
            score += cse(pawn_structure(), 1.3)
            score += cse(piece_activity(), 1.1)
            score += cse(threats(), 1.2)
            score += heatmap() * 3

            # print("MatS", material_score())
            # print("MobS", cse(mobility_score(), 1.5))
            # print("KSS", cse(king_safety(), 1.2))
            # print("PSS", cse(pawn_structure(), 1.3))
            # print("PAC", cse(piece_activity(), 1.1))
            # print("THR", cse(threats(), 1.2))
            # print("HMP", heatmap() * 3)
            # input()
